...
def random_tree_high(key, x, i, t, b, m, n_v, evals=0):
    ...
    def single_step(k):
        ...
        values, new_evals = jax.vmap(lambda sk, new_x: random_tree_high(sk, new_x, i + 1, t, b, m, n_v, evals+1))(subkeys, new_states)
        return jnp.nanmean(values), jnp.sum(new_evals)
    
    def no_exercise(_):
        value, new_evals = single_step(key)
        return value, 1 + jnp.sum(new_evals)
    
    def proceed(_):
        keys = jax.random.split(key, b // n_v)
        values, batch_new_evals = jax.vmap(single_step)(keys)
        
        continuation_value = jnp.nanmean(values)
...
def monte_carlo_tree_high(S0, b, n, m, n_v, n_chunks=1):
    start = time.time()
    ...
    try:
        for i in trange(0, n, batch_size, desc=f"{b=} {n=}"):
            ...
            batch_results, batch_evals = jax.vmap(lambda k: random_tree_high(k, S0, 0, t, b, m, n_v))(batch_keys)
        ...
        n_chunks = 3 * (int(total_bytes_needed // available_bytes) + 1)

        print(f"Retrying with {n_chunks} chunks")
        return monte_carlo_tree_high(S0, b, n, m, n_v, n_chunks)
...
if b := input():
    ...
    key = jax.random.PRNGKey(0)
    value, evals = random_tree_high(key, S0, 0, t, b, m, n_v)
    print(f"{b} {value:.3f} {evals:,} evals")
else:
    n = 10_000
    b_values = [10, 20, 50, 100, 200, 500, 1_000, 2_000]
    print("|$b$|$n$|High Estimate|95% CI|95% CI length|Nodes evaluated|Pruned (%)|Time (s)|")
    print("|-|-|-|-|-|-|-|-|")
    for b in b_values:
        b = b // n_v * n_v
        mean, CI, evals, pruned, elapsed = monte_carlo_tree_high(S0, b, n, m, n_v)
